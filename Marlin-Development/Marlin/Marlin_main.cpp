...
// 起動画面選択
#ifdef BOOT_DISPLAY
    int boot_display_mode = BOOT_DISPLAY;
#endif
...
inline void gcode_G0_G1() {
  if (IsRunning()) {
    ...
    #endif //FWRETRACT
/*
SERIAL_ECHO_START;// test
SERIAL_ECHOPGM("\n_destination[X_AXIS] :");SERIAL_ECHO(destination[X_AXIS]);
SERIAL_ECHOPGM("\n_destination[Y_AXIS] :");SERIAL_ECHO(destination[Y_AXIS]);
SERIAL_ECHOPGM("\n_destination[Z_AXIS] :");SERIAL_ECHO(destination[Z_AXIS]);
SERIAL_ECHOPGM("\n_current_position[X_AXIS] :");SERIAL_ECHO(current_position[X_AXIS]);
SERIAL_ECHOPGM("\n_current_position[Y_AXIS] :");SERIAL_ECHO(current_position[Y_AXIS]);
SERIAL_ECHOPGM("\n_current_position[Z_AXIS] :");SERIAL_ECHO(current_position[Z_AXIS]);
*/
    prepare_move();
    //ClearToSend();
  }
...
inline void gcode_G28() {
  ...
  #ifdef MESH_BED_LEVELING
//    unsigned char mbl_was_active = mbl.active;
...
//      current_position[Z_AXIS] = MESH_HOME_SEARCH_Z;
      ...
      sync_plan_position();
      mbl.active = 1;
//destination[Z_AXIS]=MESH_HOME_SEARCH_Z;
//SERIAL_ECHO_START;// test
//SERIAL_ECHOPGM("\ncurrent_position[X_AXIS] :");SERIAL_ECHO(current_position[X_AXIS]);
//SERIAL_ECHOPGM("\ncurrent_position[Y_AXIS] :");SERIAL_ECHO(current_position[Y_AXIS]);
//SERIAL_ECHOPGM("\ncurrent_position[Z_AXIS] :");SERIAL_ECHO(current_position[Z_AXIS]);
//SERIAL_ECHOPGM("\ndestination[X_AXIS] :");SERIAL_ECHO(destination[X_AXIS]);
//SERIAL_ECHOPGM("\ndestination[Y_AXIS] :");SERIAL_ECHO(destination[Y_AXIS]);
//SERIAL_ECHOPGM("\ndestination[Z_AXIS] :");SERIAL_ECHO(destination[Z_AXIS]);
//SERIAL_ECHOPGM("\nmbl.active :");SERIAL_ECHO(mbl.active);
...
void process_commands() {
  if (code_seen('G')) {
    ...
    switch(code_value_short()) {
      ...
      case 990: // M990 起動画面切り替え
        SERIAL_ECHO_START;
        switch( boot_display_mode ){
          case 1:
            SERIAL_ECHOLNPGM("Boot Display:BS-ICHIKO MODE");
            boot_display_mode = 2;
            break;
          case 2:
            SERIAL_ECHOLNPGM("Boot Display:BS-ICHIKO-TAN MODE");
            boot_display_mode = 3;
            break;
          case 3:
            SERIAL_ECHOLNPGM("Boot Display:BS LOGO MARK MODE");
            boot_display_mode = 1;
            break;
        }
        SERIAL_EOL;
...
void prepare_move() {
  ...
  #if (!defined(DELTA) && !defined(SCARA))
//SERIAL_ECHO_START;
//SERIAL_ECHOPGM("PrepareMove destination[X_AXIS] :");SERIAL_ECHO(destination[X_AXIS]);
//SERIAL_ECHOPGM("PrepareMove destination[Y_AXIS] :");SERIAL_ECHO(destination[Y_AXIS]);
//SERIAL_ECHOPGM("PrepareMove destination[Z_AXIS] :");SERIAL_ECHO(destination[Z_AXIS]);
